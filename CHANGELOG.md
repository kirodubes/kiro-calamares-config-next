...
- **Why:** Kiro Plasma is a **Wayland-only** edition. Calamares writes `KEYMAP=` (console) to
  ...
  choice (e.g. Belgian/AZERTY came up QWERTY). Verified on a live VM: setting the layout via
  `localectl` made `localectl status` report it, and a fresh Plasma login then used it.
...
- **Validation gate:** mirror to production `kiro-calamares-config` only after a full `-next`
  install run with a non-US layout confirms AZERTY on the installed desktop.
...
- `kiro_final`'s systemd-boot cleanup (step 9) now also removes
  **`kiro-grub-theme`** and the **`update-grub`** helper, alongside
  `kiro-bootloader-grub-nemesis` and `grub`. Both depend on `grub`, so they are
  listed before it in the single `pacman -R` transaction. On BIOS/GRUB installs
  they stay (update-grub is a useful helper there).
...
**What Changed**
- Added **`spice-vdagent`** to the existing **`qemu`** VM-cleanup profile (`packages: ("qemu-guest-agent", "spice-vdagent")`).

**Why**
- `spice-vdagent` (now shipped on the ISO, kiro-iso-next same date) is a QEMU/SPICE clipboard agent — useless on VMware, VirtualBox, and bare metal. SPICE lives only on **qemu/kvm**, which is exactly `qemu-guest-agent`'s lifecycle, so it folds into the `qemu` profile: **kept on kvm/qemu, stripped on bare-metal / VirtualBox / VMware**. No `disable_services`/`orphan_symlinks` needed for it — `spice-vdagentd.service`/`.socket` are `static` (no enable-symlink ever created), so `pacman -Rns` removing the package cleans the units + `/etc/xdg/autostart/spice-vdagent.desktop`.

**Files Modified**
- `usr/lib/calamares/modules/kiro_final/main.py` — `spice-vdagent` added to the `qemu` cleanup profile.
