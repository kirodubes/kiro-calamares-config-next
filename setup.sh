...
configure_git() {
    ...
    if [[ "${SCRIPT_DIR}" == */EDU*/* ]]; then
      ...
      git -C "${SCRIPT_DIR}" config --local user.email
    "erik.dubois@gmail.com"
      git -C "${SCRIPT_DIR}" remote set-url origin
    "git@github.com:erikdubois/${project}"
      log_success "Git configured — remote set to
    git@github.com:erikdubois/${project}"
    ...
    "kirodubes@gmail.com"
      git -C "${SCRIPT_DIR}" remote set-url origin
    "git@github.com-kiro:kirodubes/${project}"
      log_success "Git configured — remote set to
    git@github.com-kiro:kirodubes/${project}"
    else
      log_error "Cannot determine identity — path contains neither EDU
    nor KIRO: ${SCRIPT_DIR}"
      exit 1
    fi
}
